...
async function boot(){
  ...
  // Apply school config branding
  if(typeof SCHOOL_CONFIG !== 'undefined'){
    const sbn=document.getElementById('sb-school-name');
    if(sbn) sbn.textContent=SCHOOL_CONFIG.short_name||SCHOOL_CONFIG.name;
    const sc=document.getElementById('session-chip');
    if(sc) sc.textContent='Session '+SCHOOL_CONFIG.session;
  }
  buildNav();
  buildMobileNav();
  loadNotifs();
